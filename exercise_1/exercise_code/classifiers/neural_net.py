...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores = scores - np.expand_dims(np.max(scores, axis=1), axis=1)  # (N,C)
        probability = np.exp(scores) / np.sum(np.exp(scores), axis=1, keepdims=True)  # (N, C)
        loss = np.sum(-np.log(probability[np.arange(N), y]))
        loss = loss/N
        ...
        d_hidden_layer = np.dot(dscores, W2.T)

        d_hidden_layer[hidden_layer <= 0] = 0
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            random_indices = np.random.choice(num_train,batch_size,replace=True)
            ...
            for index in self.params:
                self.params[index] -= learning_rate*grads[index]
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    for l_rate in learning_rate:
        for r_strength in regularization_strengths:
            print("model:", count)
            network = TwoLayerNet(input_size, hidden_size, num_classes)
